...
public class ImageService {
    ...
    // 로컬 경로 설정
    private String localLocation = "/Users/hyun/Downloads/";

    public ImageService(S3Client s3Client) {
        this.s3Client = s3Client;
    }

    public String imageloadUserProfile(MultipartFile multipartFile, Long id) {
        String s3Url = null;
        try {
            ...
            // S3 URL 생성
            s3Url = String.format("https://%s.s3.%s.amazonaws.com/%s", bucketUserprofile, region, uniqueFileName);
    ...
    public void deleteImageUserProfile(String s3Url) {
        try {
            // URL에서 버킷과 키 추출
    ...
    public String imageloadChallenge(MultipartFile multipartFile, Long id) {
        String s3Url = null;
        try {
            if (multipartFile == null || multipartFile.isEmpty()) {
                throw new IllegalArgumentException("업로드할 파일이 없습니다.");
            ...
            // S3 URL 생성
            s3Url = String.format("https://%s.s3.%s.amazonaws.com/%s", bucketChallenge, region, uniqueFileName);
    ...
    public void deleteImageChallenge(String s3Url) {
        try {
            // URL에서 버킷과 키 추출
            String key = s3Url.substring(s3Url.lastIndexOf("/") + 1);
